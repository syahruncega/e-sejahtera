...
import { createAnalis, updateAnalis } from 'store/slices/analis';
...
const FormProfilAnalis = () => {
  ...
  const queryUpdateAnalis = useMutation({
    mutationFn: async (values) => {
      const { email, noHp, ...rest } = values;
      const putUser = updateUser(user.id, { email, noHp, role: user.role });
      const putAnalis = updateAnalis(profil.id, {
        userId: user.id,
        ...rest
      });
      return Promise.all([putUser, putAnalis]);
    },
    onSuccess: async () => {
      await updateSession();
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      toast.promise(
        isEdit ? queryUpdateAnalis.mutateAsync(values) : queryCreateAnalis.mutateAsync(values),
        {
          ...
          <Grid container spacing={gridSpacing}>
            ...
            <Grid item xs={12}>
              <Stack direction="row" display="flex" justifyContent="flex-end">
                <LoadingButton
                  loading={queryCreateAnalis.isLoading || queryUpdateAnalis.isLoading}
...
export default FormProfilAnalis;
